...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const createOrUpdateFiles = createTool({
      ...
      description:
        "Create or update one file in the sanbox. Call this tool once per file with a relative path and full file contents.",
      parameters: z.object({
      ...
      handler: async ({ path, content }, { step, network }) => {
        return step?.run(`create-or-update-file-${path}`, async () => {
          try {
    ...
    const terminal = createTool({
      ...
      handler: async ({ command }, { step, network }) => {
        return step?.run(`terminal-${command}`, async () => {
          const buffers = { stdout: "", stderr: "" };
          try {
            const sandbox = await connectSandbox(network.state.data.sandboxId);
    ...
    const makeTextAgent = (name: string, system: string) =>
      createAgent({ name, system, model: AI_Model });
